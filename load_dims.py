...
# dim_patient:  SCD Type 2 (History stored: A change closes the old row
#                 and inserts a new current row)
# other dims:    SCD Type 1 (update in place: codes do not change
#                 meaning, so no history is needed)
...
def load_dim_patient(cur):
    ...
    # IS DISTINCT FROM is like <> but treats NULLs correctly
    # (NULL vs NULL = no change, NULL vs value = change).
    close_sql = """
    ...
    # SCD2 step 2: insert a new current row for every staged patient that
    # has no current row. this covers brand new patients AND the newly changed ones
...
def load_dim_allergen(cur):
    # one row per (code, category, type). GROUP BY + MAX(display) makes
    # sure we insert only ONE row per key even if the source has the same
    # code with different display texts.
